...
def freq_feature(data,sig_type,fs):
	#===tonic===
	gsr_low_filter = low_pass_filter(data,fc=1.8,fs=fs, order=4)
	...
	if sig_type == 'p.':
		...
		freq_band = [[0.25,0.5],[0.5,0.75],[0.75,1],[1,1.25],[1.25,1.8]]
	elif sig_type == 't.':
	...
	#===phasic & tonic ===
	if sig_type == 'p.':
		phasic = band_pass_filter(data,lowcut=0.25, highcut=1.8, fs=fs, order=3)
		#phasic = downsample(phasic,fs=fs,nfs=4)
		f_p,pd_p = welch(phasic, fs=fs, window='hamming', nperseg=1024)
		pd_p = pd_p[band2idx(f_p,0.25,1.8)]
		f_p = f_p[band2idx(f_p,0.25,1.8)]
		feature_type, feature_type_name = get_freq_info(pd_p,f_p)
	elif sig_type == 't.':
		tonic = band_pass_filter(data,lowcut=0.05, highcut=0.25, fs=fs, order=3)
		#tonic = downsample(tonic,fs=fs,nfs=1)
		f_t,pd_t = welch(tonic, fs=fs, window='hamming', nperseg=1024)
...
def SCR_generate(signal,fs,min_amplitude,task):
	if task=='det.':
		...
		onset, peak, amp, sig = freqSCR(signal,[0.5,1.8],'band',fs,min_amplitude)
	elif task == 'bd.m.':
		onset, peak, amp, sig = freqSCR(signal,[0.3,1.8],'band',fs,min_amplitude)
	elif task == 'bd.l.':
		onset, peak, amp, sig = freqSCR(signal,[0.1,1.8],'band',fs,min_amplitude)
	elif task == 'lo.h.':
		onset, peak, amp, sig = freqSCR(signal,[0.2],'low',fs,min_amplitude)		
	elif task == 'lo.l.':
		onset, peak, amp, sig = freqSCR(signal,[0.08],'low',fs,min_amplitude)
	elif task == 'CDA.':
		onset, peak, amp, sig, sig2, sig3 = CDASCR(signal,fs,min_amplitude)
		return onset, peak, amp, sig, sig2, sig3	
	elif task == 'CVX.':
