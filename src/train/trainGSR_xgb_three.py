...
"======setting======" 
threshold = 10 #different feature number
...
def readfile(filename,task, remove_correlation=True):
...
def main():
	os.makedirs("gsr_result_{}/3_class/acc/".format(sensor), exist_ok=True)
	os.makedirs("gsr_result_{}/3_class/val/".format(sensor), exist_ok=True)
	os.makedirs("gsr_result_{}/3_class/ft/".format(sensor), exist_ok=True)
	os.makedirs("gsr_result_{}/3_class/importance/".format(sensor),exist_ok=True)
...
def trainTask(task, outfile):
	...
	for num, val in enumerate(participant_list):
		...
		X_train = np.array(X_train)
		Y_train = np.array(Y_train)
		print(X_train.shape, Y_train.shape, name.shape)
		"===========normalize==========="
	...
	"===========getresult==========="
	folder = 'gsr_result_{}/3_class/val/'.format(sensor)+outfile[:-4]+'/'
	...
	writefile('gsr_result_{}/3_class/acc/'.format(sensor)+outfile,accuracy,'w')
	writefeat('gsr_result_{}/3_class/ft/'.format(sensor)+outfile,best_feature,vote,'w')
